...
```{r} flights_1 <- flights %>%   filter(dep_delay > 1)}
```
...
```{r} flights %>%   filter(dep_delay > 1 & origin == "JFK")}
```
...
```{r} flights %>%   filter(dep_delay > 1 | origin == "JFK") # either has a departure delay > 1 OR is coming from JFK}
```
...
```{r} flights %>%   arrange(year, month, day, dep_delay)}
```

In this code below, the results are arranged by descending reaction time.

```{r} flights %>%   arrange(desc(dep_delay))}
```
...
```{r} flights %>%   distinct() # finds any duplicate values (if any) # looks like there are none!}
```
...
```{r} flights %>%   distinct(origin, dest, .keep_all=TRUE)}
```
...
```{r} flights %>%   mutate(     gain = dep_delay - arr_delay,     hours = air_time / 60,     gain_per_hour = gain / hours,     .keep = "used" # indicates that I want to keep the columns I used in the function.    )}
```

We can use `.before` and `.after` to put the columns before or after a specific column.

```{r} flights %>%   mutate(     gain = dep_delay - arr_delay,     hours = air_time / 60,     gain_per_hour = gain / hours,     .before = 1   )}
```

```{r} flights %>%   mutate(     gain = dep_delay - arr_delay,     hours = air_time / 60,     gain_per_hour = gain / hours,     .after = 1   )}
```
...
```{r} flights %>%   select(year, month, day)}
```
...
```{r} flights %>%   select(where(is.character))}
```
...
```{r} flights %>%   select(tail_num = tailnum)}
```
...
```{r} flights %>%   rename(tail_num = tailnum)}
```
...
```{r} flights %>%   relocate(time_hour, air_time)}
```

You can also specify where to relocate them.

```{r} flights %>%   relocate(     starts_with("J"), # relocates anything starting with J     .after = dep_time   )}
```
